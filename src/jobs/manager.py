...
class JobManager:
            ...
            try:
                ...
                for key, value in args.items():
                    if value is not None:
                        if key == "input_dir":
                            cmd.extend(["--input_dir", str(value)])
                        elif key == "input_files":
                            # For batch processing with comma-separated files
                            cmd.extend(["--input_files", str(value)])
                        elif key == "output":
                            cmd.extend(["--output", str(value)])
                        else:
                            cmd.extend([f"--{key}", str(value)])

                # Set output directory in job directory
                ...
                with open(log_file, 'w') as log:
                    process = subprocess.Popen(
                        ...
                        env=dict(os.environ, CONDA_DEFAULT_ENV="./env")
                    )
                ...
                if process.returncode == 0:
                    ...
                    output_files = list(job_dir.glob("*.fasta")) + list(job_dir.glob("*.json")) + list(job_dir.glob("*.npz"))
    ...
    def get_job_result(self, job_id: str) -> Dict[str, Any]:
        ...
        # Load FASTA files
        for fasta_file in job_dir.glob("*.fasta"):
            with open(fasta_file) as f:
                result_data["outputs"][fasta_file.name] = f.read()

        # Load JSON files
        for json_file in job_dir.glob("*.json"):
            with open(json_file) as f:
                result_data["outputs"][json_file.name] = json.load(f)
